...
from opencti_consts import (
    OPENCTI_CONNECTIVITY_ENDPOINT,
    OPENCTI_ERR_CONNECTIVITY_TEST,
    OPENCTI_SUCC_CONNECTIVITY_TEST,
)


class OpenctiConnector(BaseConnector):
    """
    Opencti connector class that serves as a starting point for new connectors.
    """
    ...
    def _handle_test_connectivity(self, param):
        ...
        endpoint = OPENCTI_CONNECTIVITY_ENDPOINT
        headers = {"Authorization": f"Bearer {self._api_key}"}

        ret_val, _ = self._make_rest_call(endpoint, action_result, headers=headers)

        if phantom.is_fail(ret_val):
            self.save_progress(OPENCTI_ERR_CONNECTIVITY_TEST)
            return action_result.get_status()

        self.save_progress(OPENCTI_SUCC_CONNECTIVITY_TEST)
